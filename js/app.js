(() => {
  ...
  function renderGoals(){ const arr=state.data.goals; $('#view-goals').innerHTML=`${pageHeader('Objetivos','Metas para reducir deuda y cerrar préstamos.','<button class="btn btn-primary" data-action="add-goal">+ Nuevo objetivo</button>')}<div class="three-col">${arr.length?arr.map(g=>`<div class="card"><h3>${esc(g.name)}</h3>${goalCard(g)}<button class="btn btn-danger" data-action="delete-goal" data-id="${g.id}">Eliminar</button></div>`).join(''):empty('Sin objetivos','Crea una meta medible.')}</div>`; }
  ...
  function renderReports(){
    ...
    $('#view-reports').innerHTML=`${pageHeader('Centro de análisis','Resúmenes por acreedor, préstamo, mes, tipo y estado.','<button class="btn btn-secondary" data-action="copy-report">Copiar resumen filtrado</button>')}${reportFilterForm(loans)}
